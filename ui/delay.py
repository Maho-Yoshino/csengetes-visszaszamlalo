...
class delayGUI:
		...
		def __init__(self):
			...
			ttk.Label(self.root, text="Set Delay Time (in seconds)").grid(row=0, column=0)
			ttk.Entry(self.root, textvariable=self.delayvar).grid(row=1, column=0)
			ttk.Label(self.root, text="Negative = Earlier\nPositive = Later").grid(row=2, column=0, rowspan=2)
			ttk.Button(self.root, text="Save", command=self.saveValue).grid(row=5, column=0)
