...
class delayGUI:
		...
		def __init__(self):
			...
			self.root.title(state.settings.localization.delaySetting.title)
			...
			ttk.Label(self.root, text=state.settings.localization.delaySetting.label).grid(row=0, column=0)
			ttk.Entry(self.root, textvariable=self.delayvar).grid(row=1, column=0)
			ttk.Label(self.root, text=state.settings.localization.delaySetting.help).grid(row=2, column=0, rowspan=2)
			ttk.Button(self.root, text=state.settings.localization.delaySetting.btn, command=self.saveValue).grid(row=5, column=0)
